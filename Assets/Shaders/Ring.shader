Shader "Custom/Ring"
{
	Properties
	{
		...
		_InnerRadius("Inner Radius", Range(0.0, 1.0)) = 0.9
	...
	SubShader
	{
		...
		{
			if (length(IN.worldPos - _Center) > _OuterRadius * (5 * _ScaleFactor))
			{
				discard;
			}

			if (length(IN.worldPos - _Center) < _InnerRadius * (5 * _ScaleFactor))
			{
				discard;
			}
